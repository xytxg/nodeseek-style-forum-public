...
function getLevelColorClasses(level: number): string {
  const colorMap: Record<number, string> = {
    0: "bg-slate-500 hover:bg-slate-600 text-white", // Gray for beginners
    1: "bg-blue-500 hover:bg-blue-600 text-white", // Blue for learning
    2: "bg-emerald-500 hover:bg-emerald-600 text-white", // Green for growing
    3: "bg-amber-500 hover:bg-amber-600 text-white", // Amber for active
    4: "bg-purple-500 hover:bg-purple-600 text-white", // Purple for advanced
    5: "bg-rose-500 hover:bg-rose-600 text-white", // Rose for expert
    6: "bg-indigo-600 hover:bg-indigo-700 text-white", // Indigo for master
  }

  return colorMap[level] || "bg-slate-500 hover:bg-slate-600 text-white"
}
...
export function UserLevelBadge({ userLevel, showDrumsticks = false, size = "sm" }: UserLevelBadgeProps) {
  const sizeClasses = {
    sm: "text-[10px] px-1.5 py-0", // Reduced vertical padding to achieve 80% height
    md: "text-xs px-2 py-0", // Reduced vertical padding to achieve 80% height
    lg: "text-sm px-2.5 py-1",
  }

  const liveIconSizes = {
    sm: "h-3 w-auto", // Reduced from h-4 to h-3
    md: "h-4 w-auto", // Reduced from h-5 to h-4
    lg: "h-5 w-auto", // Reduced from h-6 to h-5
  }

  if (userLevel.isSuper) {
    return (
      <div className="flex items-center gap-1">
        <LiveIcon className={liveIconSizes[size]} />
        {showDrumsticks && <span className="text-xs text-muted-foreground ml-1">🍗 {userLevel.drumsticks}</span>}
      </div>
    )
  }

  const levelColorClasses = getLevelColorClasses(userLevel.level)

  return (
    <div className="flex items-center gap-1">
      <Badge className={`${sizeClasses[size]} ${levelColorClasses} flex items-center gap-1 border-0 rounded-[2px]`}>
        <span>{getLevelDisplay(userLevel)}</span>
      </Badge>
      {showDrumsticks && <span className="text-xs text-muted-foreground ml-1">🍗 {userLevel.drumsticks}</span>}
    </div>
  )
